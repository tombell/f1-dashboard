...
import { getRaceControl } from "@/shared/api/openf1";
...
export default function RaceControl({ sessionKey }: RaceControlProps) {
  ...
  if (!sessionKey) {
    return (
      <div className="bg-f1-bg2 border border-f1-border rounded-lg flex-1 flex items-center justify-center text-f1-dim text-sm">
  ...
  return (
    <Panel
      title="Race Control"
      className="max-h-[480px] flex flex-col"
      bodyClassName="flex-1 min-h-0 overflow-y-auto overscroll-contain p-3 space-y-1.5"
    >
      {messages.length === 0 && (
